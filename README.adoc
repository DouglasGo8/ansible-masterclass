...
In automation, you often need to handle *sensitive informations* like password, sensitive keys, API Keys, tokens etc.
Ansible provides tools to *securely manage* these secrets without exposing them in plain text;
...
- Loops in Ansible let you run the same task multiple times and with different values, this helps us avoid writing the same task again

- For example, we need to install multiple packages like `httpd, `git`, and `wget` you can use the loop keyword

- We use loops to: avoid repetitive similar tasks re-writing, to handle lists likes user, packages or services, to keep playbooks clean, short and easy to maintain

.Task Loop Playbook Sample (p9.yml)
[source,yaml]
----
---
- name: Install common packages on dev servers
  hosts: dev
  become: true
...
.Run ansible over p9.yml
[source,bash]
----
ansible-navigator run p9.yml -m stdout -i /home/projects/inventory.ini -u ec2-user
#
----

.Task Loop Playbook Sample with Variable List (p10.yml)
[source,yaml]
----
---
- name: Install common packages on dev servers
...
.Run ansible over p10.yml
[source,bash]
----
ansible-navigator run p10.yml -m stdout -i /home/projects/inventory.ini -u ec2-user
# on Woker/Client EC2 Instance
dpkg -l | grep postfix
----

.Sample how to install loops over dictionaries
[source, bash]
----
ansible-navigator run playbooks/dict-loop.yml -m stdout -i /home/projects/inventory.ini -u ec2-user
----

== Section11: Task Control implementation in Ansible
...
- Conditionals let you tell Ansible to run a task only if certain are true

- Sometimes you want a task to run only when specific conditions are met - like only if a server has enough free memory, or only certain types of machines

- What cane you test?
** Variables you define in the playbook
** Variables saved using register from other tasks
** Facts gathered automatically about the host (like OS type, memory IP, etc)
...
== Section12: Deploying Files to Managed Hosts

- File Modules are used to manage files, directories, and symbolic links on remote machines
- They help automate file creation, modification, valition and delation in a reliable, repetable way
- All modules described below are part of the ansible.builtin collectin, so they don't require any extra installation

- Commonly Use File Modules

** #`file:`# _create/delete files_: directories, or symlinks; set permissions and ownership
** #`copy:`# _copy_ files from your control machine to the remote host
** #`fetch:`# _retrieve_ files from he remote host to your local machine
** #`lineinfile:`# _ensures_ a specific line is present, updated, or removed in a file, allowing you to add or modify content line-by-line
** #`blockinfile:`# _insert, update or remove_ a block of lines in a file


== Section13: Deploy Custom Files with Jinja2 Templates

Jinja2 Templates is powerful way to manage files over templates, we can write a template configuration file that is automatically customized for the managed host, Jinja2 templates is composed by multiple elements: _data, variables and expression_. Those variables and expressions are replaced with their values when the Jinja2 template is rendered. The variables used in the template can be spcified in the vars sections of the playbook. It is possible to use the manged hosts' facts as varavles on a template


.File Jinja2 sample.j2
[source, text]
----
{{ variable1 }}
---
...
.Playbook file1.yml with sample.j2 ref
[source, yaml]
----
---
hosts: server
...
tasks:
  - name: Basic Template Example
    template:
      src: sample.j2
      dest: /home/ec2-user/output/output.txt
----

.Playbook file2.yml with sample.j2 ref
[source, yaml]
----
---
hosts: server
...
tasks:
  - name: Basic Template Example
    template:
      src: sample.j2
      dest: /home/ec2-user/output/output2.txt
----
...
== Section14: Managing Complex Plays and Playbooks

When running Ansible playbooks, you can choose which systems (hosts) to target using host patterns. These patterns helps you apply tasks to the machines you want - saving time and resources, using hosts patterns

=== What are Hosts Patterns

* Host patterns are used in the hosts: field of a playbook to:
.. Include specific hosts or groups
.. Exclude certain hosts
.. Combine multiple groups

IMPORTANT: Theres is important concept do divide complex playbooks using include and import files